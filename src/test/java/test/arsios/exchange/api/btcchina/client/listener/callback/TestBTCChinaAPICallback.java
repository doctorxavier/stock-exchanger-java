package test.arsios.exchange.api.btcchina.client.listener.callback;
...
@ContextConfiguration(classes = {TestBTCChinaAPICallback.class})
//CHECKSTYLE:OFF
...
public class TestBTCChinaAPICallback {
	
	private static final Logger		LOGGER		= LoggerFactory.getLogger(TestBTCChinaAPICallback.class);
	...
	public void test() {
		...
		Date datelt = null;
		try {
			dategte = simpleDateFormat.parse("2014-09-24T00:00:00.000+0200");
			datelt = simpleDateFormat.parse("2014-09-25T00:00:00.000+0200");
		...
		Double sdprofit = 0.0;
		Double edprofit = 25.0;
		Double idprofit = 0.5;

		Double sdorderAmount = 0.0;
		Double edorderAmount = 20.0;
		Double idorderAmount = 1.0;
		...
		//Double iterations = iprofit * iorderAmount;
		...
		try {
				...
				for (Double dorderAmount = sdorderAmount; dorderAmount < edorderAmount; dorderAmount += idorderAmount) {
					...
					for (Double dvalue = sdvalue; dvalue < edvalue; dvalue += idvalue) {
						...
						for (Double dcurrency = sdcurrency; dcurrency < edcurrency; dcurrency += idcurrency) {
							...
							//Double dprofit = 11.5;
							//Double dorderAmount = 5.0;
							//Double dvalue = 1.02;
							//Double dcurrency = 6.48;
							...
							status.setAmount(new BigDecimal(dorderAmount));
							status.setCurrency(new BigDecimal(dcurrency));
							status.setProfit(new BigDecimal(dprofit));
							status.setValue(new BigDecimal(dvalue));
							...
							if (initialOrder != null && lastOrder != null) {
								...
								LOGGER.info("First order: cty: " + initialOrder.getCurrency() + ", btc: " + initialOrder.getValue() + ", balance: " + initialOrder.getBalance());
								
								LOGGER.info("Last ticker: ask: " + tickers.get(tickers.size() - 1).getAsk() + ", bid: " + tickers.get(tickers.size() - 1).getBid());
								LOGGER.info("Last order: cty: " + lastOrder.getCurrency() + ", btc: " + lastOrder.getValue() + ", balance: " + lastOrder.getBalance());
								...
								status.setInitBalance(new BigDecimal(Utilities.formatBigDecimal(initialBalance, 2)));
								status.setEndBalance(new BigDecimal(Utilities.formatBigDecimal(lastOrder.getBalance(), 2)));
								status.setCharge(new BigDecimal(Utilities.formatBigDecimal(amortization, 2)));
			...
			LOGGER.info("Start balance: " + Utilities.formatBigDecimal(startBalance, 2) + ", end balance: " + Utilities.formatBigDecimal(maxBalance, 2) + ", amortization: " + maxAmortization.subtract(BigDecimal.ONE).multiply(ONEHUNDRED, new MathContext(8, RoundingMode.DOWN)) + ", profit: " 
					+ effectiveProfit + ", orderAmountPercentage: " + effectiveOrderAmount + ", cty: " + effectiveEur + ", btc: " + effectiveBtc);
			//CHECKSTYLE:ON		
